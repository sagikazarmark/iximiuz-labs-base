...
apt-get update
apt-get install -y --no-install-recommends kubeadm=${KUBE_VERSION#v}-* kubelet=${KUBE_VERSION#v}-* kubectl=${KUBE_VERSION#v}-*
...
for image in $(kubeadm config images list --kubernetes-version=${KUBE_VERSION}); do
    crane pull $image /opt/images/kubeadm/$(echo ${image##*/} | cut -d: -f1 | awk -F/ '{print $NF}').tar
done
